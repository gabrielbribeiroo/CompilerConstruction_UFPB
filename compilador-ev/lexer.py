"""Analisador lexico para a linguagem EV (Expressoes com Variaveis).

Estende o lexer de EC1/EC2 com tres tokens novos exigidos pela secao 3
do enunciado: identificadores, sinal de igual (atribuicao/resultado) e
ponto-e-virgula (fim de declaracao).

Um identificador comeca com uma letra (maiuscula ou minuscula) e e
seguido de zero ou mais letras ou digitos. Uma sequencia de digitos
seguida imediatamente por uma letra, sem separador (ex.: "237axy"),
continua sendo um erro lexico -- o lexer reconhece os digitos como
NUMERO e, ao encontrar uma letra logo em seguida sem espaco/operador
entre eles, sinaliza erro na posicao da letra.

Fornece as duas interfaces ao parser:
...
class AnalisadorLexico:
    ...
    def _gerar_proximo(self) -> Token:
        ...
        if c.isalpha():
            return self._ler_identificador()
    ...
    def _ler_numero(self) -> Token:
        ...
        # continuam sendo erro lexico (secao 3 do enunciado: "237axy")
    ...
    def _ler_identificador(self) -> Token:
        ...
        while self._pos < len(self._fonte) and self._fonte[self._pos].isalnum():
            self._pos += 1
        return Token(TipoToken.IDENT, self._fonte[inicio:self._pos], inicio)
