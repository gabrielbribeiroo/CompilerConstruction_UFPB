#!/usr/bin/env python3
"""compev - Compilador completo para a linguagem EV (Expressoes com Variaveis).

Uso:
    python compev.py <arquivo.ev>

Le um programa EV, faz analise lexica, sintatica e semantica (verificacao
de variaveis), constroi a arvore de sintaxe abstrata e gera um arquivo
`.s` em assembly x86-64 (sintaxe GNU Assembler) pronto para ser montado
e linkado.

A saida e gravada em um arquivo de mesmo nome base, trocando `.ev` por
`.s`.

Em caso de erro lexico, sintatico, semantico ou de E/S, imprime mensagem
em stderr e encerra com codigo de saida 1. Nesses casos nenhum arquivo
de saida e criado.
"""
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print(f"uso: python {argv[0] or 'compev.py'} <arquivo.ev>", file=sys.stderr)
        return 2
